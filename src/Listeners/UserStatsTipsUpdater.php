...
class UserStatsTipsUpdater
{
    public function handle($event)
    {
        ...
        if ($order->is_tip && $order->is_successful && ! empty($order->entityObj->mediaObj)) {
            ...
            $receiverStats['amounts']['received_tips'] = Tip::query()->user($order->entityObj->userObj->_id)->successful()->sum('commission_amount');
            ...
            $senderStats['amounts']['sent_tips'] = Tip::query()->creator($order->entityObj->creatorObj->_id)->successful()->sum('amount');
